...
| Provider-level overview card | **Option (b) from `030` §4 is taken.** The provider card means "this provider's capacity", and Meta's documented limits are per TEAM, not per key. A per-subscription window promoted to provider level would be a different quantity wearing the same label. The card's existing empty state is correct | `003` §E; `quota.ts:2298-2301` |
