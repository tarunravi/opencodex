...
describe("projectUsageSummary", () => {
  ...
  test("a strict subset filter recomputes latency and effort while retaining Codex activity", () => {
    const entries: PersistedUsageEntry[] = [
      {
        ...entry({ ts: at, requestId: "kept", provider: "openai", model: "gpt-5.5", durationMs: 1_000, usageStatus: "reported", usage: { inputTokens: 10, outputTokens: 10 } }),
        firstOutputMs: 100,
        requestedEffort: "high",
        effectiveEffort: "high",
      },
      {
        ...entry({ ts: at + 10_000, requestId: "excluded", provider: "anthropic", model: "claude-opus", durationMs: 5_000, usageStatus: "reported", usage: { inputTokens: 50, outputTokens: 50 } }),
        firstOutputMs: 2_000,
        requestedEffort: "low",
        effectiveEffort: "low",
      },
    ];
    const unfiltered = summarizeUsage(entries, "30d", at + 20_000, "all", {
      activeWallMs: 12_345,
      activeTurns: 2,
      completedTurns: 3,
    });
    ...
    expect(projected.latency).toMatchObject({
      modelCallMs: 1_000,
      apiActiveMs: 1_000,
      averageTtftMs: 100,
      activeWallMs: 12_345,
    ...
    expect(projected.effortGroups).toHaveLength(1);
    expect(projected.effortGroups[0]).toMatchObject({
      provider: "openai",
      requestedEffort: "high",
      effectiveEffort: "high",
      requests: 1,
...
describe("UsageSummaryAccumulator modes", () => {
  ...
  test("exact mode preserves cross-partition request identity", () => {
    ...
    expect(summary.effortGroups[0]).toMatchObject({ requests: 1, requestShare: 100 });
  });
