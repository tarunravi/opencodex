...
| `GET /api/codex-auth/accounts/discover` · `POST /api/codex-auth/accounts/import-codexbar` | Discover and explicitly import local CodexBar-managed Codex profiles on macOS. Discovery returns masked metadata only; import validates and deduplicates before making accounts routable. |
...
| `GET /api/codex-auth/active` · `PUT /api/codex-auth/accounts/priority` | Read the effective account (including `pinned`, `pinnedAccountId`, and a known future `nextQuotaResetAt`) and set one account's selection order. |
