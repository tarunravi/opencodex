...
| Раздел | Что делает |
| --- | --- |
| **Сводка Dashboard** | Состояние онлайн, версия с временем работы, число провайдеров и сумма токенов за 30 дней; ниже — полоса защиты от перезагрузки, синхронизация моделей, свёрнутый блок **Сайдкары** и давление памяти. Старые вкладки провайдеров и моделей убраны — используйте **Providers** и **Models**. |
| **Sub-agent delegation** | На странице **Subagents**: выбор нативной/маршрутизируемой модели и необязательного уровня рассуждений, общих для руководства OpenCodex по делегированию и опциональных нативных значений подагентов Codex по умолчанию. Это не маршрутизатор отдельных порождений; см. ниже. |
| **Сайдкары** | Свёрнутый блок на дашборде: выбор модели и уровня рассуждений для веб-поиска, а также модели описания изображений. Изменения применяются со следующего запроса. |
...
| **Автозапуск Codex** | На странице **Startup**, в деталях защиты: разрешает уже установленному launcher shim Codex выполнять `ocx ensure`. Переключатель не устанавливает shim или фоновую службу. |
...
| **Subagents** | Выделение до пяти «голых» нативных или маршрутизируемых моделей с пространством имён в списке переопределений `spawn_agent`, а также выбор v1/base/v2 и лимита потоков v2 в свёрнутом блоке **Дополнительно** секции делегирования. |
| **Models** | Включение и отключение нативных GPT и маршрутизируемых моделей, настройка списков разрешённых провайдеров и лимитов контекста. Политика новых моделей, псевдонимы, перехват shadow-вызовов и лимиты окна по умолчанию находятся в свёрнутом блоке **Дополнительно**; дополнительные настройки провайдера (псевдонимы по умолчанию, пользовательские модели, лимиты окна) открываются кнопкой ⋯ в заголовке провайдера. |
...
Макет теперь один, поэтому переключать нечего. `#dashboard` — это Overview; старые закладки `#dashboard/providers` и `#dashboard/models` ведут на `#providers` и `#models`. Перезагрузка, закладка и кнопка «Назад» сохраняют выбранную страницу. **Logs** работает так же — `#logs` и `#logs/debug`. Старая закладка `#providers/workspace` теперь ведёт на `#providers`.
...
Селектор **Sub-agent delegation** на странице **Subagents** сохраняет `injectionModel` и, при желании,
