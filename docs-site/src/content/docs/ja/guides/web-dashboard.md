...
| 領域 | 機能 |
 --- | --- |
| **ダッシュボード要約** | オンライン状態、バージョンと稼働時間、プロバイダー数、直近30日トークン合計を表示し、その下に再起動保護バー、モデル同期、折りたたまれた **サイドカー** セクション、メモリ圧力が続きます。以前のプロバイダー/モデルタブはなくなったので **プロバイダー** と **モデル** を使ってください。 |
| **サブエージェント委任** | **サブエージェント** ページで、OpenCodex の委任ガイダンスとオプションの Codex ネイティブサブエージェント既定値で共有するネイティブ/ルーティングモデルと任意の推論強度を選びます。スポーンごとのルーターではありません。下記を参照してください。 |
| **サイドカー** | ダッシュボードの折りたたみセクションで、ウェブ検索モデルと強度、画像説明モデルを選択します。次回リクエストから適用されます。 |
...
| **Codex 自動起動** | **起動** ページの保護状態の詳細で、インストール済み Codex launcher shim に `ocx ensure` の実行を許可します。このトグルは shim やバックグラウンドサービスをインストールしません。 |
...
| **サブエージェント** | `spawn_agent` オーバーライド一覧にネイティブまたはルーティングモデルを最大 5 つまで優先公開し、委任セクションの **詳細設定** 折りたたみで v1/base/v2 と v2 スレッド数を設定します。 |
| **モデル** | ネイティブ GPT とルーティングモデルをオン/オフし、プロバイダー許可リストとコンテキスト上限を設定します。新規モデルポリシー、エイリアス、シャドウ呼び出しの横取り、既定ウィンドウ上限は折りたたまれた **詳細設定** に、プロバイダーごとの追加設定（既定エイリアス、カスタムモデル、ウィンドウ上限）はプロバイダーヘッダーの ⋯ ボタンにあります。 |
...
レイアウトは 1 つだけなので、切り替える設定はありません。`#dashboard` は Overview で、以前の `#dashboard/providers` と `#dashboard/models` のブックマークは `#providers` と `#models` に移動します。再読み込み・ブックマーク・戻る操作のいずれでも、表示していたページが保たれます。**Logs** も `#logs` と `#logs/debug` で同じように動作します。以前の `#providers/workspace` のブックマークは `#providers` に移動します。
